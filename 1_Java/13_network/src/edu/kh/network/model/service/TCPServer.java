...
public class TCPServer {
	...
	public void serverStart() {
		...
		int port = 8500; // port 번호는 0~65535 사이 지정 가능
						 // 단 1023번 이하는 이미 사용중인 경우가 많으니 제외한다.
		...
		clientSocket = serverSocket.accept(); // 클라이언트소켓이 서버소켓을 수락했다?
		
		// 접속한 클라이언트의 IP를 얻어와 출력
		String clientIP = clientSocket.getInetAddress().getHostAddress(); // 인터넷 주소를 얻어온다.
		System.out.println(clientIP + "가 연결을 요청함...");
		
		// 5. 연결된 클라이언트와 입출력 스트림 생성
		is = clientSocket.getInputStream(); // 클라이언트로부터 입력할수있는 통로를 얻어온다.
		os = clientSocket.getOutputStream(); // 클라이언트로부터 출력할수있는 통로를 얻어온다.
		
		// 6. 보조 스트림을 통해 성능 개선
		br = new BufferedReader(new InputStreamReader(is)); // is = 1byte
		// inputStreamReader : 문자기반 스트림과 바이트 기반 스트림 연결에 사용되는 스트림
		
		pw = new PrintWriter(os); // 얜 os만 써도 됨 왜와이??
		...
		String time = sdf.format(now); // now에 저장된 시간포맷 변경
		...
		String message = br.readLine(); // 클라이언트 메세지 한 줄을 읽어온다.
		...
		System.out.println(clientIP + "가 보낸 메세지 : " + message);
			...
			try {
				...
				// 보조 스트림 close시 연결된 기반 스트림(is, os)도 같이 close 된다.
