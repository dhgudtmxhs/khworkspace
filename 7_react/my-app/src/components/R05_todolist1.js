...
const InputName = () => {
    ...
    return(
        <div>
            <p>이름을 입력하세요.</p>
            <input type='text' onChange={ e => setName(e.target.value) }/> {/* setName(input의 value) */}
            <h3>{name}</h3>
        </div>
    )
}
...
const TodoList1 = () => {
    ...
    // 할 일을 저장할 변수
    // -> 상태가 변하면 컴포넌트 TodoList1을 리렌더링
    const [todos, setTodos] = useState([ // 객체배열
    ...
    // 할 일 입력 컴포넌트
    const InputTodo = () => {
        const [inputText, setInputText] = useState(''); // []배열, {}객체, {key:'a'} 다 가능
        ...
        const addTodo = () => {
            ...
            // 새로운 객체 배열 neewTodos를 todos에 대입
            // -> 상태 변화 인식 -> 리렌더링 진행
            setTodos(newTodos)
