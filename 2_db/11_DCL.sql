...
* 사용자 계정 : 데이터베이스에 대하여 질의, 갱신, 보고서 작성등의
                ...
                ex) mmw계정(각자 이니셜 계정) workbook 등

*/
CREATE TABLE EMPLOYEE2 AS SELECT * FROM EMPLOYEE;
SELECT AVG(SALARY) FROM EMPLOYEE;
SELECT SALARY, EMP_NAME FROM EMPLOYEE WHERE SALARY >= (SELECT AVG(SALARY) FROM EMPLOYEE);
...
CREATE USER ohs_sample IDENTIFIED BY sample1234;
        /* 사용자 계정명 */         /* 비밀 번호 */ 
-- User ohs_sample이(가) 생성되었습니다.

-- 2. 생성된 계정에 접속하기 위한 방법 추가
--> ORA-01045 : CREATE SESSION 권한을 가지고 있지 않아 로그인 실패

-- 3. [SYS 관리자 계정] 생성된  샘플 계정에 DB 접속 권한 부여
GRANT CREATE SESSION TO ohs_sample;
...
GRANT CREATE TABLE TO ohs_sample;

ALTER USER ohs_sample DEFAULT TABLESPACE SYSTEM QUOTA UNLIMITED ON SYSTEM;
...
-- ROLE : 권한의 묶음
--> 묶어둔 권한(ROLE)을 특정 개정에 부여 --> 해당 계정은 특정 권한을 가진 역할을 갖게 된다.
...
-- [SYS 관리자 계정]
GRANT CONNECT, RESOURCE TO ohs_sample;

-----------------------------------------------------------------------------------

-- 객체 권한
--> mmw_sample 계정에서 mmw 계정의 EMPLOYEE 테이블 조회

-- 1.[샘플 계정] mmw계정의 EMPLOYEE 테이블 조회
SELECT * FROM ohs.EMPLOYEE; --> EMPLOYEE 테이블(객체) 조회 권한 없음

-- 2.[이니셜 계정] 샘플 계정에 EMPLOYEE 테이블 조회 권한 부여
GRANT SELECT ON EMPLOYEE TO ohs_Employee;

-- 3.[샘플 계정] 다시 EMPLOYEE 테이블 조회
SELECT * FROM ohs.EMPLOYEE;

-- 4.[이니셜 계정] 부여했던 SELECT 권한 회수
REVOKE SELECT ON EMPLOYEE FROM ohs_sample;
