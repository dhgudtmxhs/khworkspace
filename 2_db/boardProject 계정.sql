...
-- 댓글 목록 조회
SELECT COMMENT_NO, COMMENT_CONTENT,
       TO_CHAR(C_CREATE_DATE, 'YYYY"년"MM"월"DD"일"HH"시"MM"분"SS"초"') C_CREADTE_DATE,
       BOARD_NO, MEMBER_NO, MEMBER_NICKNAME, PROFILE_IMG, PARENT_NO, COMMENT_DEL_FL
FROM "COMMENT"
JOIN MEMBER USING(MEMBER_NO)
WHERE BOARD_NO = 1524
...
-- 1001, 1002
INSERT INTO "COMMENT"
VALUES(SEQ_COMMENT_NO.NEXTVAL, '부모 댓글1', DEFAULT, DEFAULT, 1524, 1, NULL);

INSERT INTO "COMMENT"
VALUES(SEQ_COMMENT_NO.NEXTVAL, '부모 댓글2', DEFAULT, DEFAULT, 1524, 1, NULL);

INSERT INTO "COMMENT"
VALUES(SEQ_COMMENT_NO.NEXTVAL, '자식 댓글1-1', DEFAULT, DEFAULT, 1524, 1, 1001);

INSERT INTO "COMMENT"
VALUES(SEQ_COMMENT_NO.NEXTVAL, '자식 댓글1-2', DEFAULT, DEFAULT, 1524, 1, 1002);

INSERT INTO "COMMENT"
VALUES(SEQ_COMMENT_NO.NEXTVAL, '자식 댓글1-3', DEFAULT, DEFAULT, 1524, 1, 1002);

INSERT INTO "COMMENT"
VALUES(SEQ_COMMENT_NO.NEXTVAL, '자식 댓글2-1', DEFAULT, DEFAULT, 1524, 1, 1002);

INSERT INTO "COMMENT"
VALUES(SEQ_COMMENT_NO.NEXTVAL, '자식 댓글2-2', DEFAULT, DEFAULT, 1524, 1, 1002);

INSERT INTO "COMMENT"
VALUES(SEQ_COMMENT_NO.NEXTVAL, '자식 댓글2-1-1', DEFAULT, DEFAULT, 1524, 1, 1009);
...
- WHERE절이 계층형 쿼리보다 해석순서가 늦기 때문에 먼저 조건을 반영하고 싶은 경우
  FROM절 서브쿼리(인라인뷰)에서 WHERE절을 사용해줘야 한다.
...
FROM "COMMENT"
JOIN MEMBER USING(MEMBER_NO)
WHERE BOARD_NO = 1524) C
WHERE COMMENT_DEL_FL = 'N'
START WITH PARENT_NO IS NULL -- 최상위 부모
...
-- 댓글 삽입
INSERT INTO "COMMENT"
VALUES (SEQ_COMMENT_NO.NEXTVAL, #{commentContent}, DEFAULT, DEFAULT, #{boardNo}, #{memberNo}, 'xxxxxx');
        
-- 댓글 삭제
UPDATE "COMMENT"
SET COMMENT_DEL_FL = 'Y'
WHERE COMMENT_NO = '1024';
...
-- 채팅방 목록 조회  FROM MESSAGE M FROM CHATTING_ROOM R FROM MESSAGE M2 FROM CHATTING_ROOM R2
     SELECT CHATTING_NO
         ,(SELECT MESSAGE_CONTENT FROM (
            SELECT * FROM MESSAGE M2 --M2
            WHERE M2.CHATTING_NO = R.CHATTING_NO -- MESSAGE의 CHATTING NO와 CHATTINGROOM의 CHATTING NO가 같은 메세지 내용
            ORDER BY MESSAGE_NO DESC) 
            WHERE ROWNUM = 1) LAST_MESSAGE -- alias
         ,TO_CHAR(NVL( SEND_TIME --  날짜와 시간의 경우, 가장 큰 값은 가장 미래의 날짜나 시간 , Alias 
               FROM MESSAGE M -- M
               WHERE R.CHATTING_NO  = M.CHATTING_NO), CH_CREATE_DATE),  -- 번호에 해당하는 메세지가 없거나 보낸 시간이 NULL인 경우 MAX(SEND_TIME)을 NULL 대신 CH_CREATE_DATE로 대체
               'YYYY.MM.DD') SEND_TIME
         ,NVL2((SELECT OPEN_MEMBER FROM CHATTING_ROOM R2  --R2
            WHERE R2.CHATTING_NO = R.CHATTING_NO
            AND R2.OPEN_MEMBER = #{memberNo}), 
            R.PARTICIPANT,
            R.OPEN_MEMBER
            ) TARGET_NO -- nvl2문 alias
         ,NVL2((SELECT OPEN_MEMBER FROM CHATTING_ROOM R2 --R2
            WHERE R2.CHATTING_NO = R.CHATTING_NO
            AND R2.OPEN_MEMBER = #{memberNo}),
            (SELECT MEMBER_NICKNAME FROM MEMBER WHERE MEMBER_NO = R.PARTICIPANT), -- 참여자 닉네임 가져오기
            (SELECT MEMBER_NICKNAME FROM MEMBER WHERE MEMBER_NO = R.OPEN_MEMBER) -- 개설자 닉네임 가져오기
            ) TARGET_NICKNAME -- nvl2문 alias
         ,NVL2((SELECT OPEN_MEMBER FROM CHATTING_ROOM R2 --R2
            WHERE R2.CHATTING_NO = R.CHATTING_NO
            AND R2.OPEN_MEMBER = #{memberNo}),
            (SELECT PROFILE_IMG FROM MEMBER WHERE MEMBER_NO = R.PARTICIPANT), -- 참여자 이미지 가져오기
            (SELECT PROFILE_IMG FROM MEMBER WHERE MEMBER_NO = R.OPEN_MEMBER) -- 개설자 이미지 가져오기
            ) TARGET_PROFILE -- nvl2문 alias
