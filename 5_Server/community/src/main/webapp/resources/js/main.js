...
function loginValidate(){ // 로그인 유효성 검사
    ...
    if(inputEmail.value.trim().length == 0){ 
        ...
        inputEmail.value=""; // input 내용 삭제
        inputEmail.focus(); // input에 focus
        
        return false; // 기본 이벤트 제거 위해 false 반환

    }

    // 비밀번호를 입력하지 않은 경우 false를 반환
	if(inputPw.value.trim() == ""){
        alert("비밀번호를 입력해주세요.");
        
        inputPw.value=""; // input 내용 삭제
        inputPw.focus();
