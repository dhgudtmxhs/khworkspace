...
public class LoginServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		// JSP로 안한다면 out.print("<html>"); 이렇게 한 줄 씩 써야했음
		...
		//   좀 더 효율적으로 처리할 수 있는 JSP에게 넘겨줄 예정이다.
		
		// req.getRequestDispatcher("JSP경로");
		// -HttpServletRequest 객체가 생성될 때
		// 내부에 자동으로 요청발송자(RequsetDispatcher)가 같이 생성된다.
		
		// -"JSP경로" : 전달할 JSP의 경로를 작성
		
		// *** JSP 경로 작성 규칙 ***
		// - webapp 폴더를 기준으로 해서 JSP 파일까지의 모든 경로를 작성해야한다.
