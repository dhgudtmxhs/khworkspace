...
function documentWrite(){
    ...
    //document.write("<b>안녕하세요</b><br><br><h2>helloworld</h2>방가방가");
...
function setInnerText(){
    // id가 "test1인 요소를 얻어와"
    // test1 변수에 대입
    const test1 = document.getElementById("test1"); /* document는 생략 불가임 */
...
function getInnerHTML1(){
    ...
    // test2 요소 내부에 내용(태그 + 속성 + 내용) 을 읽어서 출력
...
function setInnerHTML1(){
    ...
    test2.innerHTML = "<b>innerHTML로 변경된 <br> 내용입니다.</b>";
}
...
function add(){
    ...
    //const content = area1.innerHTML; // content = "<div class='box2'></div>" 

    // 3) area1의 이전 내용 + 새로운 요소(div.box2)를 추가한다.
...
function fnConfirm(){
    ...
    // 확인
    if(confirm("버튼 배경색을 분홍색으로 바꾸시겠습니까?")){ //true -> 확인
        document.getElementById("confirmBtn").style.backgroundColor = "pink";
    }else{ // false -> 취소
        document.getElementById("confirmBtn").style.backgroundColor = "orange";
    }
...
function fnInput(){
    ...
    const idpwd = document.getElementById("area4");
    ...
    /* input은 닫는 태그가 없어서 innertext, innerhtml 사용 불가함. value로 다뤄야함 */
}
