...
// 선택된 요소 "다음"부터 매개변수로 작성된 형제관계 요소 "이전"까지를 모두 선택
// span 태그부터 h3태그까지 폰트 크기를 30px로 변경
...
$("span").on("click", function(){
    ...
    console.log( $("span") ); // 모든 스팬
    console.log( $(this) ); // 클릭한 스팬
    console.log ( this ); // 이벤트가 발생한 요소 자체

    // 이벤트 핸들러 (이벤트 발생 시 동작을 지정하는 함수)
    // 내부에 작성된 this라는 단어는 이벤트가 발생한 요소를 나타낸다.

})

// h2 태그를 클릭했을 때
// 클릭한 요소의 이전 요소의
// 배경색을 red, 글자 white로 설정
