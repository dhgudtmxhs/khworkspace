...
public class MemberView {
	...
	public void updatePw(Member loginMember) {
		...
		try {
			...
			if(result > 0) {
				...
				loginMember.setMemberPw(newPw); // 로그아웃하고 로그인 할때는 SQL에서 바껴있어서 할 필요가 없긴 함
	...
	public int secession(Member loginMember) {
		...
		if(ch == 'Y') { // 탈퇴 여부를 바꾼다 data_default의 n을 y로 -> update
			
			try {
			int result = service.secession(loginMember.getMemberNo(), memberPw);
			
			// 4. 탈퇴 Service 수행 성공 -> "탈퇴 되었습니다."
			//	  탈퇴 Service 수행 실패 -> "비밀번호가 일치하지 않습니다."
			if(result > 0) {
				System.out.println("탈퇴 되었습니다."); // -> 강제 로그아웃 시켜주기
				
				
				//loginMember = null;
				// -> 매개변수로 전달받은 값(주소 복사본)을 저장할 뿐이다.
				// -> 복사본이 사라진닫고 해도 원본(MainView의 loginMember)는
				//	  사라지지 않는다. -> 로그아웃이 안된다.
