...
function selectCommentList(){
    ...
    // 자원을 이름으로 구분(Representaional) 하여 
    ...
    .then(response => response.json()) // 응답받은 목록들 = list 여서 json으로 파싱
...
addComment.addEventListener("click", e => { // 댓글 등록 버튼이 클릭이 되었을 때
    ...
    fetch("/comment", {
        ...
        body : JSON.stringify(data) // JS 객체 JSON으로 parsing해서 보내겠다.
    })
    .then( resp => resp.text())
    .then(result => {
        if(result > 0){ // 등록 성공
...
function deleteComment(commentNo){
    ...
    if( confirm("정말로 삭제 하시겠습니까?") ){
        ...
        fetch("/comment", {
            method : "DELETE",
            headers : {"Content-Type" : "application/json"},
            body : commentNo // 값이 하나라 객체에 담아서 JSON.stringify() 할 필요가 없다.
        })
